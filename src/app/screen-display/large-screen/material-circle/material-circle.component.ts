...
@Component({
  selector: 'app-material-circle',
  templateUrl: './material-circle.component.html',
  styleUrls: ['./material-circle.component.css']
})
export class MaterialCircleComponent implements OnInit {
  private  urls = Urls;
  ...
  GetAllMaterial(city= null):  any {
    ...
    const data = this.http.get(this.urls.GetAllMaterial, {params: params})
      .toPromise()
  ...
  renderItem = function(params, api) {
    ...
    if(api.value(9)){
      ...
      return {
        ...
        z2: api.value(1) * 2,
        style: api.style({
          stroke: 'rgba(64,67,34,1)',
          fill:  'rgba(156,156,14,0.1)',
          textPosition: textPosition,
          lineWidth: 1
        }),
        styleEmphasis: api.style({
  ...
  ngOnInit() {
    this.GetAllMaterial(this.current_city).then(r => {
      this.data = r;
    ...
    this.emitService.eventEmit.subscribe((value: any) => {
      ...
      this.GetAllMaterial(value).then(r => {
        this.data = r;
        console.log(this.data);
        this.root = d3.hierarchy(this.data)                 //
          .sum(function(d) {
  ...
  onChartClick(event) {
    this.dtOptions = {
      ...
      ajax: url_main + '/m_material/get_data_by_city/' + event.value[10],
      columns: [
        {title:'序号',data:'Id'},
        {title:'名称',data:'Name'},
        {title:'数量',data:'Num'},
